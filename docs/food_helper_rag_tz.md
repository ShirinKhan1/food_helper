...
- базовые тесты для эмбеддингов, нормализации запроса и pgvector-выдачи.

Пока не реализовано:

- HTTP API;
- чатовый endpoint;
- оркестратор намерений;
- гибридный поиск;
- фильтрация по ограничениям пользователя;
- состояние диалога;
- обработка “первый / второй / третий рецепт”;
- сервис замены ингредиентов;
- сервис калорий и БЖУ;
- генерация финального ответа через LLM;
- fallback-политики;
- полноценные бизнес-функциональные тесты RAG-сценариев.
